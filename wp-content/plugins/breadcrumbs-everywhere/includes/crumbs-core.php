 ...
 **** HOW TO USE ****
	...
	2. Theme developers: You may adapt this plugin as needed. However, if you modify the core files your changes will be overwritten by future plugin
	updates. Instead, copy the entire function here, paste it into your theme's function.php file, and make your edits. Then, reference the function
	as in Step 1 above.  	
 * 
 *
 ...
 */	
		...
		if ( bp_has_profile() && !bp_is_register_page() ) : 		
		echo $startpages . " $divider " . '<a href="' . $homeurl . '/' . $page1 . '/">' . $page1 . '</a>' . " $divider " . '<a href="' . $bp->displayed_user->domain . '">' . $bp->displayed_user->fullname  . '</a>' . " $divider " . $bp->current_component  . "";				
		endif;
		
			//** For Level 5. If you want Action breadcrumbs, uncomment and use this section instead. 
		//if ( bp_has_profile() && !bp_is_register_page() ) : 		
		//echo $startpages . " $divider " . '<a href="' . $homeurl . '/' . $page1 . '/">' . $page1 . '</a>' . " $divider " . '<a href="' . $bp->displayed_user->domain . '">' . $bp->displayed_user->fullname  . '</a>' . " $divider " . $bp->current_component . " $divider_sub " . $bp->current_action . "";				
		//endif;
		
		
		// Show on Members list page (level 1)
		// Since 1.2 added is_page() 
		// Show on all pages to catch top-level add-ons				
		if ( !bp_is_blog_page() && ( is_page() || is_page($page1) || is_page($page2) || is_page($page3) || is_page($page4) || is_page($page5) ) && !bp_has_profile() && !bp_is_single_item() ) :
    	...
    	if ( get_query_var('paged') ) {
     	if ( bp_is_blog_page() && !(is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author()) ) echo ' (Page' . ' ' . get_query_var('paged') . ')';
					}
